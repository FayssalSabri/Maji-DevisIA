export const calculateCosts = (specs, params) => {
  ...
  const materialRate = params.materialRates[specs.material?.type] || 1.2;
  ...
  if (
    ...
    volumeDm3 =
      (specs.dimensions.length * specs.dimensions.width * specs.material.thickness) / 1000000;
  ...
  const holesPerimeter = holesList.reduce(
    (sum, h) => sum + Math.PI * (h.diameter || 0) * (h.quantity || 0),
    0
  );

  let cuttingLengthMm =
    specs.dimensions?.cuttingLength > 0
  ...
  const bendingCost =
    (bendingTimeMin / 60) * (params.machineRates?.bending || 65) * machineOverhead;

  // 5. Surface Treatment
  let surfaceTreatmentCost = 0;
  const surfaceAreaM2 =
    specs.dimensions?.developedSurface ||
    ((specs.dimensions?.length || 0) * (specs.dimensions?.width || 0)) / 1000000;

  if (surfaceAreaM2 >= 0.0004) {
    surfaceTreatmentCost = 3.0;
  }
  ...
  const totalMachineTimeMin =
    setupLaser + setupBending + cuttingTimeMin + holesTimeMin + bendingTimeMin;
  const laborCost = (totalMachineTimeMin / 60) * (params.laborRate || 35);

  // Totals
  const subtotal =
    materialCost + cuttingCost + holesCost + bendingCost + surfaceTreatmentCost + laborCost;
