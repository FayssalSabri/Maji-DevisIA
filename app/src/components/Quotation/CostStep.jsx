...
export const CostStep = () => {
  ...
  const isLowThreshold = costs.subtotal < 5.0;
          ...
          <div className="anomaly-alert fade-in" style={{ marginBottom: '16px' }}>
            ...
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: '12px', opacity: 0.85 }}>
                Le coût de production ({costs.subtotal.toFixed(2)} €) est inférieur au seuil minimum
                de 5,00 €. Vérifiez les quantités du lot ou les paramètres machine.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="cost-line">
            <div className="cost-line-label">
              <Package size={16} /> Matière ({state.currentWizard.specs.material.type} —{' '}
              {costs.details?.calculatedMass?.toFixed(3) || 0} kg)
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.material} suffix=" €" />
            </div>
          </div>
          <div className="cost-line">
            <div className="cost-line-label">
              <Scissors size={16} /> Découpe Laser (
              {costs.details?.totalCuttingLengthMm?.toFixed(0) || 0} mm)
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.cutting} suffix=" €" />
            </div>
          </div>
          <div className="cost-line">
            <div className="cost-line-label">
              <Activity size={16} /> Pliage ({costs.details?.totalBends || 0} plis × 1,50 €)
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.bending} suffix=" €" />
            </div>
          </div>
          <div className="cost-line">
            <div className="cost-line-label">
              <Paintbrush size={16} /> Traitement de Surface
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.surfaceTreatment} suffix=" €" />
            </div>
          </div>
          <div className="cost-line">
            <div className="cost-line-label">
              <Users size={16} /> Main d'œuvre / Réglages (
              {costs.details?.totalMachineTimeMin?.toFixed(1)} min)
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.labor} suffix=" €" />
            </div>
          </div>
          ...
          <div className="cost-line" style={{ background: 'var(--bg-tertiary)' }}>
            ...
            <div className="cost-line-value">
              <AnimatedCounter value={costs.subtotal} suffix=" €" />
            </div>
          </div>
          <div className="cost-line">
            <div className="cost-line-label">
              Marge Commerciale ({(state.parameters.defaultMargin * 100).toFixed(0)}%)
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.margin} suffix=" €" />
            </div>
          </div>
          <div className="cost-line" style={{ background: 'var(--bg-tertiary)', fontWeight: 600 }}>
            <div className="cost-line-label">Total H.T.</div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.total} suffix=" €" />
            </div>
          </div>
          <div className="cost-line">
            <div className="cost-line-label">TVA (20%)</div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.vatAmount} suffix=" €" />
            </div>
          </div>
          <div className="cost-line cost-total">
            <div className="cost-line-label">
              <TrendingUp size={16} /> TOTAL TTC
            </div>
            <div className="cost-line-value">
              <AnimatedCounter value={costs.totalTTC} suffix=" €" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  updateParam(null, 'defaultMargin', parseFloat(e.target.value) / 100)
                }
              />
            </div>
            <input
