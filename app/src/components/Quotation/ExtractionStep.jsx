...
export const ExtractionStep = () => {
  ...
  const steps = [
    {
      icon: FileSearch,
      title: 'Analyse OCR du document',
      desc: 'Lecture de la cartouche et des annotations'
    },
    {
      icon: ScanLine,
      title: 'Extraction dimensionnelle',
      desc: 'Reconnaissance des cotes et du volume'
    },
    { icon: Layers, title: 'Détection des opérations', desc: 'Identification des trous et plis' },
    {
      icon: BrainCircuit,
      title: 'Modélisation des coûts',
      desc: 'Préparation des paramètres de chiffrage'
    }
  ];
    ...
    <div className="fade-in split-layout">
      ...
      <div className="mobile-hidden" style={{ display: 'flex', flexDirection: 'column' }}>
        <h3 style={{ fontSize: '14px', marginBottom: '12px' }}>
          Fichier en cours d'analyse : {state.currentWizard.file?.name}
        </h3>
                    ...
                    {status === 'active' && (
                      <div className="extraction-progress">
                        <div
                          ...
                          style={{ width: '60%', animation: 'pulse 1s infinite alternate' }}
                        ></div>
                      </div>
                    )}
                  </div>
